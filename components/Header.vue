<template>
  <div>
    <div
      class="sticky_header w-full top-0 z-99 bg-white transition-all duration-300 ease-in-out"
    >
      <!-- Top bar -->
      <div
        class="bg-white md:py-0 shadow-md border-b border-gray-400 md:border-b-0 md:shadow-none"
      >
        <div
          v-if="topbar"
          class="border-b-[1px] border-gray-300 py-2 transition-all duration-300"
        >
          <div
            class="container w-11/12 mx-auto flex items-center justify-between md:flex-row-reverse"
          >
            <!--social icons-->
            <ul class="flex gap-2 items-center justify-center">
              <li v-for="item in socialIcons" :key="item.name">
                <a class="social_icon" :href="item.href">
                  <i :class="item.icon"></i>
                </a>
              </li>
            </ul>
      ...
      <!-- Menu part same as before -->
      <div class="bg-white md:bg-white md:border-b-0">
        <div class="header container w-11/12 mx-auto">
          <nav
            class="block w-full md:flex md:justify-between md:items-center md:py-3"
          >
            <div
            ...
            <div
              id="menu"
              class="bg-white text-green-700 md:text-white text-md top-0 fixed z-100 -left-60 w-full flex flex-col gap-2 max-w-0 h-[100%] transition-all duration-500 ease-in-out md:static md:flex-row md:max-w-full md:h-auto md:bg-transparent md:justify-between md:gap-4"
            >
              <div class="md:hidden text-center shadow-md p-3">
                <h2 class="font-bold text-black mb-1">Care2Training</h2>
              ...
              <ul
                class="py-2 px-6 flex flex-col gap-[2px] md:flex-row md:gap-1 md:py-1 text-md md:items-center"
              >
                <li
                ...
                >
                  ...
                  <ul
                    class="absolute z-50 bg-white shadow-md md:shadow-none md:pt-7 p-4 border border-gray-300 md:border-0 w-full md:w-56 overflow-hidden invisible translate-y-[80%] opacity-0 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 ease-in-out"
                    v-if="item.dropdown"
                  >
                    <li
                      class="relative py-1"
